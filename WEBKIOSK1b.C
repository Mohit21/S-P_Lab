...
//for dos.h not included in the Linux library

void delay( int Sec){
clock_t ticks1=clock(),ticks2=ticks1;
clock_t target=Sec*CLOCKS_PER_SEC;
while((ticks2/CLOCKS_PER_SEC-ticks1/CLOCKS_PER_SEC)< target){
	ticks2=clock();
	}
}
...
struct time_table1
{
struct time_table tab;
struct time_table1 *next;
}*temp1;
typedef struct time_table1 node1;
node1 *start1,*trav1;
void displayt();
//----------------------------------------------------------------------------
void academic();
...
//settextstyle(3,0,3);
outtextxy(200,130,"JAYPEE INSTITUTE OF INFORMATION TECHNOLOGY");
//settextstyle(3,0,1);
outtextxy(200,150,"Deemed to be University under section 3 of UCG Act");
getch();
    // closegraph();
	      }
//Login------------------------------------------------------------------------------------------------------------
...
//	delay(2);
	for(i=0;i<=360;i++)
	{
		...
		//settextstyle(3,0,2);
		outtextxy(225,410,"Loading....please Wait");
		//delay(10);
	}
	closegraph();
	}
...
//floodfill(getmaxx()/3,getmaxy()/100,15);
  //     settextstyle(3,0,3);
       outtextxy(220,130,"JAYPEE INSTITUTE OF INFORMATION TECHNOLOGY");
    //   settextstyle(3,0,1);
       outtextxy(200,150,"Deemed to be University under section 3 of UCG Act");
     //	int gd=DETECT,gm,
	//delay(500);
...
setcolor(BLACK);
//settextstyle(1,0,9);
outtextxy(50,140,"LOGIN");
//settextstyle(1,0,9);
outtextxy(70,160,"PAGE");
//fseek(fl,0,SEEK_END);
gotoxy(90,160);
printf("Institue:");
fflush(stdin);
...
for(j=0;j<=2;j++)
{
	if((strcasecmp(pwd,s_pwd[j])==0) && (strcasecmp(uid,s_uid[j]))==0)
	{
		flag=1;
		break;
	}
